...
class HMMSpeechRecog(object):
    def __init__(self,  features,labels,spoken,index,filespath=Path('train/audio/'), val_p=0.2):
        ...
        self.left = int(index/6*len(self.features))
        self.right = int((index+1)/6*len(self.features))
    ...
    def test(self, save_path=None):
        ...
        for i in range(self.left, self.right):
            ...
            if self.speechmodels[id].Class == labels[i]:
                count +=1
...
if __name__ == "__main__":
    ...
    cross_validation(features, labels , spoken)
