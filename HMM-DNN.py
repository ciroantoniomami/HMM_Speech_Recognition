...
def get_data_dict_train(data):
    data_dict = {}
    count = 0
    key = 1
    mat = []
    word = ["down","go","left","no","off","on","right","stop","up","yes"]
    for line in data:
        if line=="\n":
...
def sg_train(digits, train_data):
    ...
    for digit in digits:
        data = np.vstack([train_data[id] for id in train_data.keys() if digit in str(id)])
...
def hmm_train(digits, train_data, sg_model, nstate, niter):
    ...
    for digit in digits:
        ...
        data = [train_data[id] for id in train_data.keys() if digit in str(id)]
...
if __name__ == '__main__':
    ...
    digits = ["down","go","left","no","off","on","right","stop","up","yes"]
    uniq_state_dict = {}
    i=0
    for digit in digits:
    ...
    with open(args.train) as f:
        train_data = get_data_dict_train(f.readlines())
    # for debug
    if args.debug:
    ...
    with open(args.test) as f:
        test_data = get_data_dict_test(f.readlines())
    # for debug
    if args.debug:
    ...
    if args.mode == 'mlp':
        ...
        hmm_model = hmm_train(digits, train_data, sg_model, args.nstate, 3)
